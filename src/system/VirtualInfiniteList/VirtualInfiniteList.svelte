<script lang="ts">
  ...
  $: visible = initialized
    ...
    : []
  $: if (mounted && $changes && $type) onChange($type, ...$changes)
  ...
  async function onTop(newers: T[], olders: T[]) {
    ...
    const previousDom = rows[diff]
      ? rows[diff].firstElementChild // after second time
      : rows[diff - 1] // first time
      ? rows[diff - 1].firstElementChild
      : undefined
    if (
  ...
  async function onScroll() {
    ...
    const reachedTop = Math.floor(viewport.scrollTop) === 0
    const reachedBottom = Math.ceil(viewport.scrollHeight - viewport.scrollTop) === viewport.clientHeight
    if (direction !== Direction.bottom && reachedTop) dispatch('infinite', { on: Direction.top })
    if (direction !== Direction.top && reachedBottom) dispatch('infinite', { on: Direction.bottom })
    if (reachedTop) dispatch('reached:top')
    if (reachedBottom) dispatch('reached:bottom')
  ...
  export async function scrollTo(offset: number) {
    if (!initialized || !viewport) return
  ...
  export async function scrollToIndex(index: number, options = { align: 'top' }) {
    ...
    searching = true
    const { found, top: t, itemRect } = await search(index, uniqueKey)
    if (!found) {
      searching = false
      return false
    }
    ...
    switch (options.align) {
      ...
      case 'bottom': {
        top = t - viewport.getBoundingClientRect().height + ( itemRect?.height || 0 )
        break
      }
      case 'center': {
        top = t - viewport.getBoundingClientRect().height / 2 + ( itemRect?.height || 0 )
      }
    }
    if (top === 0) top = 1
  ...
  function getTop(index: number, uKey: string) {
    const element = contents.querySelector(
      `#svelte-virtual-infinite-list-items-${(items[index] as never)[uKey]}`
    )

...
import { commandsCtx, createCmd, createCmdKey, schemaCtx, type ThemeInputChipType } from '@milkdown/core';
...
export const link = createMark<string, LinkOptions>((utils, options) => {
  return {
    ...
    schema: () => ({
      ...
      toDOM: (mark) => ['a', { ...mark.attrs, rel: 'external', class: utils.getClassName(mark.attrs, id) }],
      parseMarkdown: {
